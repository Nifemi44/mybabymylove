...
/** A small confetti burst used once the surprise heart is tapped. */
function SurpriseBurst() {
  const pieces = useMemo(() => {
    ...
    return Array.from({ length: 20 }).map((_, i) => {
      ...
      const ty = Math.round(Math.sin(angle) * distance - 50);
      ...
      {pieces.map((p) => (
        <span
          ...
          style={{
            ...
            fontSize: p.size,
            color: p.color,
            animation: `intro-confetti-burst 1s ease-out ${p.delay}ms forwards`,
...
/** Hidden interactive surprise on the front page — tap the heart. */
  ...
  return (
    <div className="relative mt-10 flex flex-col items-center">
      {!revealed ? (
        ...
        >
          <span className="surprise-pulse text-3xl text-rose">♥</span>
          <span className="font-heading text-[11px] uppercase tracking-[0.3em] text-wine/50 transition-colors group-hover:text-wine">
            tap for a little surprise
          </span>
        </button>
      ) : (
        <div className="surprise-pop relative mt-2 max-w-md rounded-2xl border border-gold/30 bg-cream/90 px-6 py-6 text-center shadow-xl">
          <SurpriseBurst />
          <p className="relative font-script text-2xl text-wine">
...
function Index() {
  ...
  return (
    <>
      <div aria-hidden="true" className="site-ambient-bg" />
      <AmbientSparkles />
      <BackgroundMusic />
      <IntroAnimation />
      ...
      <section className="relative min-h-screen overflow-hidden">
        {/* ambient glow blobs — purple + hot-pink + crimson */}
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          <div
            className="a-floaty a-pulse absolute -left-24 -top-24 size-96 rounded-full"
            style={{
              background: "radial-gradient(circle, rgba(239,93,168,.55), transparent 70%)",
            }}
          />
          <div
            className="a-floaty2 a-pulse absolute top-1/3 -right-32 size-[28rem] rounded-full"
            style={{
              background: "radial-gradient(circle, rgba(107,33,146,.45), transparent 70%)",
            }}
          />
          <div
            className="a-pulse absolute bottom-0 left-1/4 size-80 rounded-full"
            style={{
              background: "radial-gradient(circle, rgba(224,51,92,.25), transparent 70%)",
            }}
          />
        </div>
      ...
      {/* ---------- LOVE DECLARATION ---------- */}
      <section className="relative bg-cream/95 py-24 sm:py-32">
      ...
      <section className="relative bg-cream/95 py-24 sm:py-28">
      ...
      <section className="relative overflow-hidden py-24 sm:py-32">
        <div
          ...
          style={{
            background:
              "radial-gradient(120% 120% at 50% 100%, #f3d9f7 0%, #f6e4ff 70%)",
          }}
        />
