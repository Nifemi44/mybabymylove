...
import { registerBackgroundAudio } from "@/lib/background-audio-bus";
...
export function BackgroundMusic({ autoStart = false }: { autoStart?: boolean }) {
  ...
  // Let other components (like the teddy-bear voice note) duck this volume.
  useEffect(() => {
    if (audioRef.current) registerBackgroundAudio(audioRef.current, 1);
  }, [src]);

  // Self-healing: if the browser/device pauses this out from under us for any
  // reason other than the user tapping the toggle (tab switch, phone lock,
  // a stray autoplay-policy hiccup), quietly try to resume it — the music is
  // meant to keep playing repeatedly, not fall silent on its own.
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;

    const resumeIfWanted = () => {
      if (wantsPlaying.current && audio.paused) {
        audio.play().catch(() => {
          // Will retry on the next visibility/focus/gesture event.
        });
      }
    };
    ...
    audio.addEventListener("pause", resumeIfWanted);
    document.addEventListener("visibilitychange", onVisibility);
    window.addEventListener("focus", resumeIfWanted);
    window.addEventListener("click", resumeIfWanted);
    window.addEventListener("touchstart", resumeIfWanted, { passive: true });

    return () => {
      audio.removeEventListener("pause", resumeIfWanted);
      document.removeEventListener("visibilitychange", onVisibility);
      window.removeEventListener("focus", resumeIfWanted);
      window.removeEventListener("click", resumeIfWanted);
      window.removeEventListener("touchstart", resumeIfWanted);
    };
  }, [src]);
